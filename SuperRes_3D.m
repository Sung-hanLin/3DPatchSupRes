...
c_input = c_mri_bic_SS_12;   %single slice C13 image (low-res)
...
N = 11;  %patch size (actual patch  = (N*2)+1)
...
for k = 1:size(down_p,1)
    for s = 1:size(down_H,3)
        down_H_shift(:,:,s) = Fn_immove(down_H(:,:,s),shift_y,shift_x);
        p_shift(k,:,:,s) = Fn_immove(squeeze(down_p(k,:,:,s)),shift_y,shift_x);
    end
end
down_p = p_shift;
down_H = down_H_shift;
...
parfor s = zzi:zzj
    for j = yyi:yyj
        for i = xxi:xxj
            w(i,j,s,:,:,:) = Fn_Cal_weight_H_3D_vector_v2([i,j,s],K,p_1,N,Nz,h_low_1, L_Vi, L_Viz);        
        end
    end
end
t = toc;
...
[input_2_1H_re, t] = Fn_SR_recon_s2(w,input_1, input_2_1H, N, Nz, xxi,xxj,yyi,yyj,zzi,zzj);
input_2 = input_2_1H_re*1;

%Mean Correction
iteration = 1;
error = zeros(1,iteration_target, 'single');
[input_2,error(1)] = Fn_MeanCorrection(input_2, input_1,c_low, c_input, TargDim,TargSize, SS_expan_L, SS_expan_Lz);
...
while iteration < iteration_target
    ...
    input_1 = input_2;
    input_2_1H = zeros(size(input_1), 'single');
    [input_2_1H, t] = Fn_SR_recon_s2(w,input_1, input_2_1H, N, Nz, xxi,xxj,yyi,yyj,zzi,zzj);

    input_2 = input_2_1H*1;
    [input_2 ,error(iteration)] = Fn_MeanCorrection(input_2, input_1,c_low, c_input, TargDim,TargSize, SS_expan_L, SS_expan_Lz);
    fprintf('#iter. = %d; Recon. t. =  %d sec; error = %d\n', iteration, t, error(iteration));
end
...
save( filename,'c_super_res','error', 'c_init', 'iteration', 'K', 'N', 'Nz', 'input_1', 'input_2', ...
    'weighting_patch_cal_time', 'shift_x', 'shift_y', 'down_H', 'c_low');
